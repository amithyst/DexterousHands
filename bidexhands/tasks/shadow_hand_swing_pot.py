...
class ShadowHandSwingPot(BaseTask):
    """
    This class corresponds to the SwingPot task. This environment involves two hands and a dual handle 
    pot, we need to use two hands to hold and swing the pot together
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object_start_pose.r = gymapi.Quat().from_euler_zyx(0, 0, 0) #修改之处从1.57变为0
    ...
    def compute_reward(self, actions):
        ...
        Compute the reward of all environment. The core function is compute_hand_reward(
            ...
            self.max_episode_length, self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.pot_right_handle_pos, self.pot_left_handle_pos, 
    ...
    def compute_observations(self):
        ...
        self.pot_right_handle_pos = self.rigid_body_states[:, 26 * 2, 0:3].clone()
        self.pot_right_handle_rot = self.rigid_body_states[:, 26 * 2, 3:7].clone()
        self.pot_right_handle_pos = self.pot_right_handle_pos + quat_apply(self.pot_right_handle_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0)
        self.pot_right_handle_pos = self.pot_right_handle_pos + quat_apply(self.pot_right_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0.062)
        self.pot_right_handle_pos = self.pot_right_handle_pos + quat_apply(self.pot_right_handle_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0)

        self.pot_left_handle_pos = self.rigid_body_states[:, 26 * 2 + 1, 0:3].clone()
        self.pot_left_handle_rot = self.rigid_body_states[:, 26 * 2 + 1, 3:7].clone()
        self.pot_left_handle_pos = self.pot_left_handle_pos + quat_apply(self.pot_left_handle_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0)
        self.pot_left_handle_pos = self.pot_left_handle_pos + quat_apply(self.pot_left_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0)
        self.pot_left_handle_pos = self.pot_left_handle_pos + quat_apply(self.pot_left_handle_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.06)
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        422 - 424	pot right handle position
        425 - 427	pot left handle position
        """
    ...
    def reset_target_pose(self, env_ids, apply_reset=False):#目标角度与位置
        """
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                # self.add_debug_lines(self.envs[i], self.pot_right_handle_pos[i], self.pot_right_handle_rot[i])
                # self.add_debug_lines(self.envs[i], self.pot_left_handle_pos[i], self.pot_left_handle_rot[i])
...
def compute_hand_reward(
    ...
    max_episode_length: float, object_pos, object_rot, target_pos, target_rot, pot_right_handle_pos, pot_left_handle_pos,
    ...
    参数：
        rew_buf (tensor): 当前时间所有环境的奖励缓冲区

        reset_buf (tensor): 当前时间所有环境的重置缓冲区

        reset_goal_buf (tensor): 当前时间所有环境的仅目标重置缓冲区

        progress_buf (tensor): 当前时间所有环境的进度缓冲区

        successes (tensor): 当前时间所有环境的成功缓冲区

        consecutive_successes (tensor): 当前时间所有环境的连续成功缓冲区

        max_episode_length (float): 此环境中的最大回合长度

        object_pos (tensor): 物体的位置

        object_rot (tensor): 物体的旋转

        target_pos (tensor): 目标的位置

        target_rot (tensor): 目标的旋转

        pot_left_handle_pos (tensor): 锅的左手柄位置

        pot_right_handle_pos (tensor): 锅的右手柄位置

        left_hand_pos, right_hand_pos (tensor): 双手的位置

        right_hand_ff_pos, right_hand_mf_pos, right_hand_rf_pos, right_hand_lf_pos, right_hand_th_pos (tensor): 右手五根手指的位置

        left_hand_ff_pos, left_hand_mf_pos, left_hand_rf_pos, left_hand_lf_pos, left_hand_th_pos (tensor): 左手五根手指的位置

        dist_reward_scale (float): 距离奖励的比例

        rot_reward_scale (float): 旋转奖励的比例

        rot_eps (float): 旋转计算的epsilon

        actions (tensor): 当前时间所有环境的动作缓冲区

        action_penalty_scale (float): 动作惩罚奖励的比例

        success_tolerance (float): 成功判定的容忍度

        reach_goal_bonus (float): 物体到达目标时的奖励

        fall_dist (float): 当物体距离Shadowhand较远时，判断为坠落

        fall_penalty (float): 物体坠落时的惩罚

        max_consecutive_successes (float): 最大连续成功次数

        av_factor (float): 计算连续成功的平均因子

        ignore_z_rot (bool): 是否需要忽略z轴旋转，通常用于某些特定物体（例如笔）
    """
    # 手到物体的距离
    ...
    # 手指到锅把的距离
    right_hand_finger_dist = (torch.norm(pot_right_handle_pos - right_hand_ff_pos, p=2, dim=-1) + torch.norm(pot_right_handle_pos - right_hand_mf_pos, p=2, dim=-1)
                            + torch.norm(pot_right_handle_pos - right_hand_rf_pos, p=2, dim=-1) + torch.norm(pot_right_handle_pos - right_hand_lf_pos, p=2, dim=-1) 
                            + torch.norm(pot_right_handle_pos - right_hand_th_pos, p=2, dim=-1))
    left_hand_finger_dist = (torch.norm(pot_left_handle_pos - left_hand_ff_pos, p=2, dim=-1) + torch.norm(pot_left_handle_pos - left_hand_mf_pos, p=2, dim=-1)
                            + torch.norm(pot_left_handle_pos - left_hand_rf_pos, p=2, dim=-1) + torch.norm(pot_left_handle_pos - left_hand_lf_pos, p=2, dim=-1) 
                            + torch.norm(pot_left_handle_pos - left_hand_th_pos, p=2, dim=-1))
    # 手中物体与目标物体的方向对齐
    quat_diff = quat_mul(object_rot, quat_conjugate(target_rot))
    rot_dist = 3.0 * torch.asin(torch.clamp(torch.norm(quat_diff[:, 0:3], p=2, dim=-1), max=1.0))
    ...
    # 掉落惩罚：当物体离开手的范围时，给予惩罚
    reward = torch.where(object_pos[:, 2] <= 2.5, reward + fall_penalty, reward)

    resets = torch.where(object_pos[:, 2] <= 0.35, torch.ones_like(reset_buf), reset_buf)

    # 判定哪些环境达到目标并更新成功计数
    successes = torch.where(successes == 0, 
                    torch.where(rot_dist < 0.8, torch.ones_like(successes), successes), successes)
    ...
    if max_consecutive_successes > 0:
        reward = torch.where(progress_buf >= max_episode_length, reward + 0.6 * fall_penalty, reward)
