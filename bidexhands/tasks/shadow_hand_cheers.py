...
class ShadowHandCheers(BaseTask):
    """
    This class corresponds to the PourWater task. This environment involves two hands and a bottle, 
    we need to Hold the cup with one hand and the bucket with the other hand, and pour the water 
    from the cup into the bucket. In the practice task in Isaac Gym, we use many small balls to 
    simulate the water
    ...
    def compute_reward(self, actions):
        ...
        Compute the reward of all environment. The core function is compute_hand_reward(
            ...
            self.max_episode_length, self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.cup_handle_pos, self.bucket_handle_pos, self.cup_spout_pos,
    ...
    def compute_observations(self):
        ...
        self.cup_handle_pos = self.rigid_body_states[:, 26 * 2, 0:3]
        self.cup_handle_rot = self.rigid_body_states[:, 26 * 2, 3:7]
        self.cup_handle_pos = self.cup_handle_pos + quat_apply(self.cup_handle_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.0)
        self.cup_handle_pos = self.cup_handle_pos + quat_apply(self.cup_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0.15)
        self.cup_handle_pos = self.cup_handle_pos + quat_apply(self.cup_handle_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.0)

        self.cup_spout_pos = self.rigid_body_states[:, 26 * 2, 0:3].clone()
        self.cup_spout_rot = self.rigid_body_states[:, 26 * 2, 3:7].clone()
        self.cup_spout_pos = self.cup_spout_pos + quat_apply(self.cup_spout_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.0)
        self.cup_spout_pos = self.cup_spout_pos + quat_apply(self.cup_spout_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * -0.2)
        self.cup_spout_pos = self.cup_spout_pos + quat_apply(self.cup_spout_rot, to_torch([0, 0, 1], device=self.device).repeat(self.num_envs, 1) * 0.07)
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[:, obj_obs_start + 13:obj_obs_start + 16] = self.cup_handle_pos
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                self.add_debug_lines(self.envs[i], self.cup_handle_pos[i], self.cup_handle_rot[i])
                self.add_debug_lines(self.envs[i], self.bucket_handle_pos[i], self.bucket_handle_rot[i])
                self.add_debug_lines(self.envs[i], self.cup_spout_pos[i], self.cup_spout_rot[i])
...
def compute_hand_reward(
    ...
    max_episode_length: float, object_pos, object_rot, target_pos, target_rot, cup_handle_pos, bucket_handle_pos, cup_spout_pos,
    ...
    参数:
        rew_buf (tensor): 当前时刻所有环境的奖励缓冲区

        reset_buf (tensor): 当前时刻所有环境的重置缓冲区

        reset_goal_buf (tensor): 当前时刻所有环境的仅目标重置缓冲区

        progress_buf (tensor): 当前时刻所有环境的进度缓冲区

        successes (tensor): 当前时刻所有环境的成功次数缓冲区

        consecutive_successes (tensor): 当前时刻所有环境的连续成功次数缓冲区

        max_episode_length (float): 该环境中的最大回合长度
        ...
        cup_handle_pos (tensor): 马克杯把手的位置

        bucket_handle_pos (tensor): 水桶把手的位置

        cup_spout_pos (tensor): 马克杯嘴的位置
        ...
        rot_eps (float): 计算旋转时的偏差

        actions (tensor): 当前时刻所有环境的动作缓冲区

        action_penalty_scale (float): 动作惩罚奖励的比例

        success_tolerance (float): 成功判定的容忍度

        reach_goal_bonus (float): 物体到达目标时给予的奖励

        fall_dist (float): 当物体远离Shadowhand时被判断为掉落的距离

        fall_penalty (float): 物体掉落时给予的惩罚

        max_consecutive_successes (float): 最大连续成功次数

        av_factor (float): 计算连续成功次数的平均因子

        ignore_z_rot (bool): 是否需要忽略z轴的旋转，通常用于某些特定物体（例如笔）
    """
    # 手与物体之间的距离
    # goal_dist = torch.norm(target_pos - object_pos, p=2, dim=-1)
    # goal_dist = target_pos[:, 2] - object_pos[:, 2]

    # right_hand_dist = torch.norm(cup_handle_pos - right_hand_pos, p=2, dim=-1)
    # left_hand_dist = torch.norm(bucket_handle_pos - left_hand_pos, p=2, dim=-1)

    right_hand_finger_dist = (torch.norm(cup_handle_pos - right_hand_ff_pos, p=2, dim=-1) + torch.norm(cup_handle_pos - right_hand_mf_pos, p=2, dim=-1)
                            + torch.norm(cup_handle_pos - right_hand_rf_pos, p=2, dim=-1) + torch.norm(cup_handle_pos - right_hand_lf_pos, p=2, dim=-1) 
                            + torch.norm(cup_handle_pos - right_hand_th_pos, p=2, dim=-1))
    ...
    up_rew = torch.where(right_hand_finger_dist < 0.7,
                    torch.where(left_hand_finger_dist < 0.7,
                                    0.6 - torch.norm(bucket_handle_pos - cup_spout_pos, p=2, dim=-1) * 2, up_rew), up_rew)
    ...
    resets = torch.where(bucket_handle_pos[:, 2] <= 0.3, torch.ones_like(reset_buf), reset_buf)
